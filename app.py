...
<style>
  ...
  div[data-testid="column"] .stButton > button {
    ...
    white-space: pre-line; transition: opacity .15s;
  }
  div[data-testid="column"] .stButton > button:hover { opacity: .82; }

  /* avail — 초록 */
  div[data-testid="column"] .stButton > button.avail,
  div[data-testid="column"] .stButton > button[data-status="avail"] {
...
if "selected_tbl" not in st.session_state: st.session_state.selected_tbl = None  # 클릭 선택 중인 자리
...
if sel and sel != st.session_state.my_table:
    ...
    <div class="detail-box">
      <div style="font-size:16px;font-weight:700;color:#0F6E56;margin-bottom:12px;">
        🪑 선택한 자리 정보
      </div>
...
# ── 좌석 배치도 ───────────────────────────────────────────────
st.markdown("### 좌석 배치도")
st.caption("🟢 예약 가능  |  ⬛ 사용 중  |  🟩 내 자리  |  자리를 클릭하면 상세 정보가 표시됩니다")
...
for zone_key, zone_info in ZONES.items():
    ...
    for row_start in range(0, len(tables), cols_n):
        ...
        for col, tbl in zip(cols, row_tbls):
            ...
            rem = reservations.get(f"{tid}__remaining")

            # 버튼 라벨
            if status == "mine":
                m, s = divmod(rem or 0, 60)
                label = f"🟩 {tid}\n{tbl['seats']}인석\n⏱ {m:02d}:{s:02d}"
            elif status == "taken":
                m, s = divmod(rem or 0, 60)
                label = f"⬛ {tid}\n{tbl['seats']}인석\n⏱ {m:02d}:{s:02d}"
            ...
            with col:
                # 색상: mine=진한초록, taken=회색 비활성, avail=연초록
                if status == "mine":
                    st.markdown(f"""
                    <style>
                    div[data-testid="column"]:has(button[kind][aria-label*="{tid}"]) button {{
                        background:#1D9E75 !important;color:#fff !important;border-color:#0F6E56 !important;
                    }}
                    </style>""", unsafe_allow_html=True)
                elif status == "avail":
                    st.markdown(f"""
                    <style>
                    div[data-testid="column"]:has(button[aria-label*="{tid}"]) button {{
                        background:#E1F5EE !important;color:#0F6E56 !important;border-color:#1D9E75 !important;
                    }}
                    </style>""", unsafe_allow_html=True)

                clicked = st.button(label, key=f"tbl_{tid}", disabled=(status == "taken"))
